...
class 设计窗口(QMdiSubWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.keyReleaseEvent = lambda e: self.事件_设计组件键盘开放(e)
    ...
    def 工具条_对齐工具(self, 对齐方法):
        ...
        ## 历史记录
        组件库 = self.组件方块数组[self.当前选中的组件[-1].property("_方块id")][2]

        if 对齐方法 == '左对齐':
            属性名称 = "左边"
            属性值 = 组件库.导出为json属性()[属性名称]
            self.操作记录.开始记录()
            ...
            for 组件 in self.当前选中的组件:  # type: QPushButton
                组件库 = self.组件方块数组[组件.property("_方块id")][2]
                左边s.append(组件库.导出为json属性()['左边'])
                新组件.append([组件库.导出为json属性()['左边'], 组件])
            新组件 = sorted(新组件, key=lambda x: x[0])
            最大左边 = max(左边s)
            最小左边 = min(左边s)
            左边相距 = (最大左边 - 最小左边) / (len(self.当前选中的组件) - 1)
            k = 0
            self.操作记录.开始记录()
            for x in 新组件:  # type: QPushButton
                组件 = x[1]
                组件库 = self.组件方块数组[组件.property("_方块id")][2]
                属性值 = 左边 = 最小左边 + k * 左边相距
                if k == 0 and k == (len(self.当前选中的组件) - 1):
                    k = k + 1
                    continue
                k = k + 1
                self.操作记录.添加("修改组件属性", {"组件库": 组件库, "属性名称": 属性名称, "属性值": 属性值}, _修改组件属性, _修改组件属性恢复)
            ...
            for 组件 in self.当前选中的组件:  # type: QPushButton
                组件库 = self.组件方块数组[组件.property("_方块id")][2]
                顶边s.append(组件库.导出为json属性()['顶边'])
                新组件.append([组件库.导出为json属性()['顶边'], 组件])
            新组件 = sorted(新组件, key=lambda x: x[0])
            最大顶边 = max(顶边s)
            最小顶边 = min(顶边s)
            顶边相距 = (最大顶边 - 最小顶边) / (len(self.当前选中的组件) - 1)
            k = 0
            self.操作记录.开始记录()
            for x in 新组件:  # type: QPushButton
                组件 = x[1]
    ...
    def 事件_设计组件被放开(self, e: QMouseEvent, 组件: QPushButton):
        ...
        if self.移动组件:
            ...
            def _修改组件属性(传递参数):
                ...
                左边n = 左边x + x - self.开始x
                顶边n = 顶边x + y - self.开始y
                组件.setGeometry(左边n, 顶边n, 宽度x, 高度x)
