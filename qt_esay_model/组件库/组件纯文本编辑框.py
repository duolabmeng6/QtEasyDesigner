# 实现 纯文本编辑框组件的构建 包括 创建组件 修改属性 导出属性 删除组件
import json
import sys
import PySide6
...
if __name__ == "__main__":
    ...
    # 创建组件
    组件纯文本编辑框 = 组件纯文本编辑框(w)
    组件纯文本编辑框.创建组件("纯文本编辑框", 顶边=10, 左边=100, 宽度=100, 高度=100)
    组件纯文本编辑框.修改组件属性("事件被点击", "事件" + 组件纯文本编辑框.对象.objectName() + "被点击")
    组件纯文本编辑框.修改组件属性("内容", "事件" + 组件纯文本编辑框.对象.objectName() + "被点击")

    # 导出组件属性
    # 导出数据 = 组件纯文本编辑框.导出组件属性()
    导出数据 = 组件纯文本编辑框.导出为json属性()
    # 导出为 json 格式 打印出来
    print(json.dumps(导出数据, indent=4, ensure_ascii=False))
    # 删除组件
    组件纯文本编辑框.删除组件()

    组件纯文本编辑框.创建组件("纯文本编辑框", 组件属性=导出数据)
