# 记录操作记录对组件 实现恢复撤消的功能
# 定义方法 开始记录 提交记录 恢复记录 撤消记录 加入操作记录
...
class 历史记录类(object):
    ...
    def 添加(self, 操作类型, 传递数据, 开始函数, 撤消函数):
        传递数据 = 开始函数( 传递数据)
        self.历史记录缓冲区.append({
            "操作类型": 操作类型,
            "传递数据": 传递数据,
    ...
    def 撤消记录(self):
        ...
        # print("--------------------撤消记录", self.当前记录位置)
        操作组 = self.生效历史记录.pop()
        self.撤消的记录.append(操作组)
        # print(数据)
        self.打印当前记录()
        self.当前记录位置 -= 1
